  ...
  const handleFinishSetup = async () => {
    if (companySize && user) {
      ...
      const { error } = await supabase
        .from('users')
        .upsert({
          uid: user.id, // Include the user's UID
    ...
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50 p-4">
      ...
      {step === 2 && (
        <div className="bg-white rounded-lg shadow-md p-6 w-full max-w-md">
          ...
          <p className="mb-4">We need to ask you to connect to the database.</p>
          <p className="mb-4 text-gray-600">
            💡 JavaScript Snippet currently available in Beta. Don't pay a thing until we collect enough info!
          </p>
      ...
      {step === 3 && (
        <div className="bg-white rounded-lg shadow-md p-6 w-full max-w-md">
          ...
          <button
            onClick={handleFinishSetup} // Send data to Supabase and redirect
            className="w-full bg-indigo-600 text-white font-semibold py-2 rounded-lg transition duration-200 hover:bg-indigo-700"
          >
            Go to Dashboard
          </button>
